...
// map with all delimiters
  ...
  function parseMultiplyDivide (scope) {
    ...
    while (operators[token] !== undefined) {
      ...
      getToken();
      params = [node, parseCombination(scope)];
      node = new OperatorNode(name, fn, params);
    }
   ...
   * @private
   */
  function parseCombination (scope) {
    var node, operators, name, fn, params;

    node = parsePermutation(scope);

    operators = { // might create problems with function, variables...as c is being used as an operator..???
      'c': math.combination,
	  'C': math.combination
    };
    ...
    while (operators[token] !== undefined) {
      ...
      getToken();
      params = [node, parsePermutation(scope)];
      node = new OperatorNode(name, fn, params);
    }
   ...
   * @private
   */
  function parsePermutation (scope) {
    var node, operators, name, fn, params;
